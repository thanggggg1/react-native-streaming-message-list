...
export const usePlaceholderState = config => {
  const $ = _c(29);
  const debounceMs = config?.debounceMs ?? DEFAULT_DEBOUNCE_MS;
  const [debouncedPlaceholderHeight, setDebouncedPlaceholderHeight] = useState(0);
  ...
  const [whitespacePhase, setWhitespacePhase] = useState("none");
